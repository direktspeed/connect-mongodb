...
var Store = require('connect').session.Store
	...
	, _collection = null
	, _defaults = {host: '127.0.0.1', port: 27017, dbname: 'dev', collection: 'Session', reapInterval: 60 * 1000};
...
var MONGOSTORE = module.exports = function MongoStore(options, callback) {
	...
	if (options.url) {
		...
		options.db = new mongo.Db(
			...
			new mongo.Server(url.hostname || _defaults.host, parseInt(url.port) || _defaults.port),
			{w: -1}
		);
	}
	...
	if (options.server_config) {
		...
		db = new mongo.Db(options.dbname || _defaults.dbname, server_config, {w:-1});
	}
...
MONGOSTORE.prototype.get = getSession = function (sid, cb)
{
	...
	if (!_collection)
	{
		setTimeout(function(){getSession(sid, cb)}, 50);
		return;
	}
...
MONGOSTORE.prototype.set = setSession = function (sid, sess, cb) {
	...
	if (!_collection)
	{
		setTimeout(function(){setSession(sid, sess, cb)}, 50);
		return;
	}
